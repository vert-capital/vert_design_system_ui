...
import { dayIntervalsType, modeType, IEvent, IConfig, IPeriod, IDay, IEventsWeek } from '@/utils/types/calendar';
  ...
  methods: {
    ...
    filterOutFullDayEvents() {
      ...
      for (const scheduleEvent of this.events) {
        if (scheduleEvent.time.start.match(DATE_TIME_PATTERN)) {
          allOtherEvents.push(scheduleEvent);
        } else if (
          scheduleEvent.time.start.match(DATE_PATTERN)
        ) {
    ...
    setDays() {
      const days = this.time
        ...
        .map((day: Date) => {
          ...
          const events = this.events.filter((event: IEvent) => {
      ...
      this.days = [
        {
          ...
          events: this.events.filter((event: IEvent) => {
            return (
    ...
    handleEventWasDragged(event: IEvent) {
